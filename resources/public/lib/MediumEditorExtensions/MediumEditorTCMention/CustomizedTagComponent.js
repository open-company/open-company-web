...
function CustomizedTagComponent(props) {
  ...
  var mappedUsers = props.users.map(function (user, i) {
    if (user["name"] && user["name"].toLowerCase().indexOf(currentText) !== -1) return Object.assign(user, { "selectedKey": "name" });else if (user["first-name"] && user["first-name"].toLowerCase().indexOf(currentText) !== -1) return Object.assign(user, { "selectedKey": "first-name" });else if (user["last-name"] && user["last-name"].toLowerCase().indexOf(currentText) !== -1) return Object.assign(user, { "selectedKey": "last-name" });else if (user["slack-username"] && user["slack-username"].toLowerCase().indexOf(currentText) !== -1) return Object.assign(user, { "selectedKey": "slack-username" });else if (user["email"] && user["email"].toLowerCase().indexOf(currentText) !== -1) return Object.assign(user, { "selectedKey": "email" });else return user;
  });
  var selectedUsers = mappedUsers.filter(function (user) {
    return !!user["selectedKey"];
  });
      ...
      selectedUsers.map(function (user, i) {
        ...
        return React.createElement(
          ...
          { "key": "user-" + (user["user-id"] + "-" + user["email"]),
            ...
            "data-user-id": user["user-id"],
            "data-slack-username": user["slack-username"],
            onClick: function(){props.selectMentionCallback("@" + user[user["selectedKey"]], user);} },
