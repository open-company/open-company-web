...
(defmethod dispatcher/action :revision-remove [db [_ topic as-of]]
  (let [slug (keyword (router/current-company-slug))
        company-data (dispatcher/company-data)
        ...
        should-remove-section (zero? (count revisions))
        ...
        with-fixed-topics (if should-remove-section
                            (dissoc company-data (keyword topic))
                            (assoc-in company-data [(keyword topic) :revisions-data] new-revisions))]
