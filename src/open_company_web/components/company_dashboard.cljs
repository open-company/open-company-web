...
(defcomponent company-dashboard [data owner]
  ...
  (init-state [_]
    {:navbar-editing false
     ...
     :columns-num (if (responsive/is-mobile-size?)
                    (responsive/mobile-columns-num)
                    (responsive/columns-num))})

  (did-mount [_]
    (utils/after 100 #(set! (.-scrollTop (.-body js/document)) 0))
    (when-not (:read-only (dis/company-data data))
      (get-new-sections-if-needed owner))
    (events/listen js/window EventType/RESIZE (fn [_] (om/update-state! owner #(merge % {:columns-num (if (responsive/is-mobile-size?)
                                                                                                        (responsive/mobile-columns-num)
                                                                                                        (responsive/columns-num))
                                                                                         :card-width (if (responsive/is-mobile-size?)
                                                                                                       (responsive/mobile-dashboard-card-width)
