...
(rum/defcs wrt < rum/reactive
  ...
  (let [item-id (:uuid activity-data)
        ...
        seen-users (into [] (map #(assoc % :seen true) (:reads read-data)))
        seen-ids (set (map :user-id seen-users))
        all-ids (set (map :user-id team-users))
        unseen-ids (clojure.set/difference all-ids seen-ids)
        unseen-users (into [] (map (fn [user-id]
                                    (first (filter #(= (:user-id %) user-id) team-users)))
                               unseen-ids))
        all-users (into [] (concat seen-users unseen-users))
